...
const onSubmitHandler=async(e) => {
  ...
  try {
    setLoading(true)
    const prompt=`Write an article about ${input} int ${selectedLength.text}`
...
<div className='mt-3 flex gap-3 flex-wrap sm:max-w-9/11'>
    ...
    (
        ...
        } className={`text-xs px-4 py-1 border rounded-full cursor-pointer ${selectedLength.text===item.text?'bg-blue-50 text-blue-700 font-bold':'text-gray-500 border-gray-300'}`}>{item.text}</span>
    )
    )}
     </div>
     <br/>
